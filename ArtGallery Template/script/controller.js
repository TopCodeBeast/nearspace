...
$( "#gogo" ).click(async function() {
    ...
    try{
        ...
        let host_addr =  wallet.getAccountId(); 
        ...
        let cidV1=await deployMetaVerse(dtype,themes[indx].id,title,"","",host_addr, `${title.toLowerCase().replace(" ","")}_${wallet.getAccountId().split(".")[0]}.pool.eclectica.testnet` ,ns_token);
        
        setprogress("Metaverse uploaded sucessfully on ipfs...")

        setprogress("Initiating art gallery smart contract...")
        
        await deplyContract(title.toLowerCase().replace(" ",""),`https://${cidV1}.ipfs.infura-ipfs.io/`); // await inti_contract(symbol, title, uri)
